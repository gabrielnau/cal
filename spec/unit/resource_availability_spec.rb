...
describe ResourceAvailability do
  ...
  before :all do
    @resource_av = ResourceAvailability.new(Resource.create!(name: "car"))
  end

  it "is by default unavailable" do
    period = 1.day.from_now.to_date..1.month.from_now.to_date
    expect(@resource_av.available_during?(period)).to be_false
  end

  describe "#available_on?" do
    it "returns true if the resource is available the given day" do
      @resource_av.set_on = 1.day.from_now.to_date
      expect(@resource_av.available_on?(1.day.from_now.to_date)).to be_true
    end

    it "returns false if the resource is unavailable the given day" do
      @resource_av.set_on = 10.day.from_now.to_date
      expect(@resource_av.available_on?(1.day.from_now.to_date)).to be_false
    end
  end
  ...
  describe "#available_during?" do
    before do
      ...
      @resource_av.available_during = @period
    end

    it "returns true if the resource is available each day of the given period" do
      expect(@resource_av.available_during?(@period)).to be_true
    end
    ...
    it "returns false if there is at least one day where the resource is unavailable" do
      ...
      expect(@resource_av.available_during?(overlapping_period)).to be_false
    end
  end

  describe "#available_during=" do
    it "sets availability for each day of the given period" do
      @resource_av.available_during = 1.day.from_now.to_date..2.days.from_now.to_date
      expect(@resource_av.available_on?(1.day.from_now)).to be_true
      expect(@resource_av.available_on?(2.day.from_now)).to be_true
    end
  end
  ...
  describe "#availability_rate_during" do
    ...
    it "calculates availability during a given month" do
      today = Time.now
      expect(@resource_av.availability_rate_during(today)).to eq(0)
      @resource_av.set_on = 2.days.from_now
      expected_rate = 1/Time.days_in_month(today.month, today.year)
      expect(@resource_av.availability_rate_during(today)).to eq(expected_rate)
    end
  end
